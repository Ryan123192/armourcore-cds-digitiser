"""Step 01 — Border detection: FAST marker method (template + ink_lab).

Calls ``rectify_with_markers_fast`` from
``src/armourcore_cds/phase1/marker_rectify_fast.py``.

This is the recommended Step-1 entry point.  Previous methods
(``run_baseline.py``, ``run_marker_method.py``, ``run_marker_method_robust.py``)
remain alongside it for side-by-side comparison.

    python tools/pipeline_dev/step01_border_detection/run_marker_method_fast.py
"""
...
def main() -> None:
    ...
    run_dir = make_run_dir("step01_border_detection", "marker_fast")
