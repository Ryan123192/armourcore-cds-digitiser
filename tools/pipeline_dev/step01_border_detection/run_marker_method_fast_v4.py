"""Step 01 — Border detection: FAST v4 (geometric rescue for weak markers).

Calls ``rectify_with_markers_fast_v4`` from
``src/armourcore_cds/phase1/marker_rectify_fast_v4.py``.

Key change vs v3: when one of the 4 chosen candidates has an area far
smaller than the others (typically because a shadow has reduced the
Lab-red signal to a tiny fragment), v4 predicts the marker's true
position from the 3 strong neighbours via parallelogram closure, then
re-searches the actual marker with a larger ROI anchored on the
predicted spot.  Easy cases that v3 already handled are unaffected
(no suspect flag fires).

    python tools/pipeline_dev/step01_border_detection/run_marker_method_fast_v4.py
"""
...
from armourcore_cds.phase1.marker_rectify_fast_v4 import (
    rectify_with_markers_fast_v4, detect_markers_fast_v4,
...
def _run_one(case_path: Path, out_subdir: Path) -> dict:
    ...
    t0 = time.time()
    try:
        result = rectify_with_markers_fast_v4(
            img, paper_w_mm=PAPER_W_MM, paper_h_mm=PAPER_H_MM,
            px_per_mm=DEFAULT_PX_PER_MM, debug_dir=None,
        )
        info["status"] = "ok"
        info["candidate_count"] = result.candidate_count
        info["rescued_labels"] = result.rescued_labels
        info["markers"] = {
            label: {
        ...
        overlay = img.copy()
        for label, det in result.markers.items():
            colour = (0, 200, 255) if label in result.rescued_labels else (0, 255, 0)
            ...
            cv2.putText(
                overlay, tag, (int(cx + 18), int(cy - 18)),
                cv2.FONT_HERSHEY_SIMPLEX, 1.4, colour, 4,
            )
        ...
        for _lbl, pt in zip(("TL", "TR", "BR", "BL"), result.src_quad):
            cv2.circle(overlay, (int(pt[0]), int(pt[1])), 22,
                       (0, 255, 255), -1)
        tag = (
            f"{case_path.stem}  |  cands={result.candidate_count}  "
            f"|  rescued={result.rescued_labels or 'none'}"
        )
        overlay = label_image(overlay, tag, height_px=52, scale=0.95)
...
def main() -> None:
    ...
    if not corpus:
        ...
        sys.exit(1)
    run_dir = make_run_dir("step01_border_detection", "marker_fast_v4")
    print(f"Run dir: {run_dir.relative_to(REPO)}\n")
    print(f"{'case':<28s} {'elapsed':>7s}  {'markers':>7s}  cands  rescued  status")
    print("-" * 80)
    ...
    for case in corpus:
        ...
        rescued = info.get("rescued_labels", []) or []
        rescued_str = ",".join(rescued) if rescued else "-"
        print(f"{case.stem:<28s} {info['elapsed_s']:>6.2f}s  "
              f"{n_markers:>4d}/4   {n_cands:>3}  {rescued_str:<8s} "
              f"{info['status']}")
    ...
    summary_payload = {
        ...
        "n_rescued_cases": sum(
            1 for s in summaries if s.get("rescued_labels")
        ),
        "cases": summaries,
    }
