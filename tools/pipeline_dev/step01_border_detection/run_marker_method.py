"""Step 01 — Border detection: NEW marker-based method on BLUE_* corpus.

Runs the new ``rectify_with_markers`` from
``src/armourcore_cds/phase1/marker_rectify.py`` on every test file and
saves a parallel folder structure to ``run_baseline``'s, so the two
approaches can be compared side-by-side:

  data/outputs/pipeline_dev/step01_border_detection/<stamp>_marker_method/
    <stem>/
      overlay.png                -- marker boxes + inner-corner quad on input
      rectified.png              -- final perspective-warped output
      info.json                  -- markers found, inner-corner coords, timings
    summary_overlays.png         -- 4xN grid of overlays
    summary_rectified.png        -- 4xN grid of rectified outputs
    summary.json                 -- per-file headline stats

The existing baseline run is untouched in its own folder.  The
existing ``src/armourcore_cds/phase1/boundary_detection.py`` is also
untouched; this new test only uses the new module.

    python tools/pipeline_dev/step01_border_detection/run_marker_method.py
"""
...
from armourcore_cds.phase1.marker_rectify import (
    rectify_with_markers, PAPER_W_MM, PAPER_H_MM, DEFAULT_PX_PER_MM,
)
...
def _run_one(case_path: Path, out_subdir: Path) -> dict:
    """Run marker-based rectification on one image.  Always writes
    overlay + info.json; writes rectified.png when detection succeeds."""
    ...
    try:
        ...
        # Overlay: marker boxes (green) + inner-corner quad (cyan)
        overlay = img.copy()
        for label, det in result.markers.items():
            cv2.polylines(overlay, [det.box.astype(np.int32)], True,
                          (0, 255, 0), 6)
            cx, cy = det.centre_xy
        ...
        for lbl, pt in zip(("TL", "TR", "BR", "BL"), result.src_quad):
            cv2.circle(overlay, (int(pt[0]), int(pt[1])), 22,
                       (0, 255, 255), -1)

        banner = (
            f"{case_path.stem}  |  markers: "
            + " ".join(sorted(result.markers.keys()))
        )
        overlay = label_image(overlay, banner, height_px=52, scale=1.0)
...
def main() -> None:
    ...
    run_dir = make_run_dir("step01_border_detection", "marker_method")
    ...
    for case in corpus:
        ...
        n_markers = len(info.get("markers", {})) if info.get("status") == "ok" else 0
        ...
        rect_path = out_subdir / "rectified.png"
        if rect_path.exists():
    ...
    if overlay_tiles:
        ...
        print(f"\nSummary overlays:  {run_dir / 'summary_overlays.png'}")
    if rectified_tiles:
        cv2.imwrite(
            str(run_dir / "summary_rectified.png"),
            grid_montage(rectified_tiles, cols=4, tile_max_dim=600),
        )
        print(f"Summary rectified: {run_dir / 'summary_rectified.png'}")
