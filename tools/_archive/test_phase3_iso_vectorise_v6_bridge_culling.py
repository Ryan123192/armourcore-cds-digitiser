...
def method_F_closed_centerline(
...
) -> tuple[np.ndarray, int, np.ndarray, np.ndarray, list]:
    ...
    *prune_min_branch_px* — short skeleton side-branches (junction → endpoint
    ...
    # compartments).
    # ALSO record each bridge's curve + length so downstream code can
    # culling-validate them individually (drop redundant ones).
    centerline = skel.copy()
    bridges_mask = np.zeros_like(skel)
    bridge_curves: list[tuple[np.ndarray, float]] = []
    for i, j in pairs:
        A  = descriptors[i]["pos"]
        B  = descriptors[j]["pos"]
        ...
        bridge_curves.append((curve, dist))

    return centerline, len(pairs), bridges_mask, skel, bridge_curves
...
def adaptive_method_F(
    ...
    for k in kernel_sizes:
        centerline, n_bridges, bridges_mask, pre_skel, bridge_curves = (
            method_F_closed_centerline(
    ...
    # ------------------------------------------------------------------
    # Bridge culling: try removing largest bridges first.  If removing a
    # bridge keeps the loop set valid (same count, same total area to
    # within tolerance), that bridge was redundant — drop it.  Stops on
    # first bridge whose removal breaks closure.
    #
    # This catches "cross-stub shortcut" bridges that pass facing /
    # obstacle / same-component checks but visually cut off a corner.
    # ------------------------------------------------------------------
    pre_skel       = best["pre_skel"]
    bridge_curves  = best["bridge_curves"]
    n_b_before     = len(bridge_curves)

    if bridge_curves:
        # Baseline loops with ALL bridges
        baseline_n     = best["n_loops"]
        baseline_area  = best["total_area"]

        # Sort bridges by length DESCENDING (largest first)
        idx_by_len = sorted(
            range(n_b_before), key=lambda i: -bridge_curves[i][1]
        )

        keep = [True] * n_b_before
        n_culled = 0

        def _rebuild(keep_mask):
            cl = pre_skel.copy()
            bm = np.zeros_like(pre_skel)
            for k_idx, (curve, _) in enumerate(bridge_curves):
                if not keep_mask[k_idx]:
                    continue
                cv2.polylines(cl, [curve], False, 255, 1, cv2.LINE_8)
                cv2.polylines(bm, [curve], False, 255, 1, cv2.LINE_8)
            band_ = render_uniform_band(cl, band_thickness)
            loops_ = find_inside_edge_contours(
                band_,
                min_area_abs_px=min_loop_abs_px,
                min_area_relative=min_loop_rel,
                bridge_mask=bm,
        ...
        for idx in idx_by_len:
            keep[idx] = False
            _, _, test_loops = _rebuild(keep)
            test_n     = len(test_loops)
            test_area  = sum(float(cv2.contourArea(c)) for c in test_loops)

            # Acceptable: same number of loops AND total area didn't
            # shrink (removing an artefact often INCREASES area because
            # the cut-off corner is now included in the main loop).
            if test_n >= baseline_n and test_area >= baseline_area * 0.95:
                # Bridge was redundant — keep removed; update baseline
                # so subsequent removals build on this
                baseline_n    = test_n
                baseline_area = test_area
                n_culled += 1
            else:
                # Removal broke closure — restore this bridge
                keep[idx] = True

        if n_culled > 0:
            centerline_final, bridges_mask_final, _ = _rebuild(keep)
            ep_x, _ = _skel_endpoints(centerline_final)
            final_n_ep = len(ep_x)
            n_bridges_final = sum(1 for k in keep if k)
            print(f"  Bridge cull: dropped {n_culled} redundant of "
                  f"{n_b_before} (kept {n_bridges_final})")
            return (
                centerline_final, n_bridges_final,
                best["k"], final_n_ep, baseline_n,
                bridges_mask_final,
            )
...
def main() -> None:
    ...
    if args.close_kernel is not None:
        centerline, n_bridges, bridges_mask, _, _ = method_F_closed_centerline(
