    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if sA < facing_threshold or sB < facing_threshold:
                continue
            # Obstacle check: reject chord that cuts through unrelated trace.
            if _chord_obstacle_score(trace_binary, Ai, Bj) > obstacle_threshold:
                continue
            candidates.append((dist, i, j))

    # Greedy match: SHORTEST chord first.  Plus a non-crossing constraint
    # — reject any bridge whose chord crosses an already-accepted bridge.
    # Real legitimate bridges (short, near gaps) get accepted first; wrong
    # bridges (often long diagonals across the tool interior) are
    # geometrically blocked by the legit ones they cross.
