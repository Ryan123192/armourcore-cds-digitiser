...
def find_inside_edge_contour(band: np.ndarray) -> np.ndarray | None:
    """Find the largest interior hole's contour in a closed band — this
    is the inside edge of the tool tracing.
    """
    ...
    # A hole/inner contour has parent != -1
    inner = [c for i, c in enumerate(contours) if hierarchy[0][i][3] != -1]
    if not inner:
        return None
...
def export_dxf_smooth(
    ...
    """Save the closed curve as a series of cubic-Bezier SPLINE entities
    in DXF.  Each segment is represented as a SPLINE with degree=3 and
    standard Bezier knots — Affinity Designer reads these as smooth nodes
    with handles.
    ...
    # Bezier knot vector for degree-3, 4-control-point spline:
...
def export_svg_smooth(
    ...
    """Save a smooth Bezier closed path as plain SVG."""
    h, w = image_size
...
def export_svg_with_layers(
    ...
    """Save self-contained SVG with three layers (bottom → top):
      ...
      1. ``original_image``    — input PNG, hidden by default but available
                                 in the layer stack for reference.
      2. ``gap_fixed_image``   — repaired/uniform-band PNG, visible at
                                 ``fixed_opacity`` so the vector can be
                                 visually compared against the cleaned
                                 raster.
      3. ``tool_outline``      — the smooth-Bezier closed path, stroke
                                 centered on the path (Affinity default
                                 + ``vector-effect="non-scaling-stroke"``
                                 so the line stays thin under zoom).
    ...
    svg = (
        ...
        f'  </g>\n'
        f'  <!-- Layer 3 (top): smooth-Bezier tool outline (centre-aligned stroke) -->\n'
        f'  <g id="tool_outline">\n'
        f'    <path d="{d}" fill="none" stroke="red" '
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    # 4. Inside-edge contour (interior hole of the band)
    # ------------------------------------------------------------------
    inner_contour = find_inside_edge_contour(band)
    if inner_contour is None:
        print("ERROR: no interior hole detected in the band — the centerline "
              "may not be fully closed.  Try a larger --close-kernel.")
        sys.exit(1)
    print(f"Inside-edge contour: {len(inner_contour)} raw points")

    # ------------------------------------------------------------------
    # 5. RDP simplify → Catmull-Rom Bezier segments → smooth outputs
    # ------------------------------------------------------------------
    simplified = cv2.approxPolyDP(inner_contour, args.rdp, closed=True).reshape(-1, 2)
    print(f"After RDP epsilon={args.rdp}: {len(simplified)} nodes")

    # Centripetal Catmull-Rom + corner cusps → smooth Bezier segments
    # that hug the polygon tightly, with sharp corners preserved.
    pts_f64 = simplified.astype(np.float64)
    bezier_segments = _bezier_segments_smart(
        pts_f64,
    ...
    n_corners = sum(
        1 for i in range(len(pts_f64))
        if (
            (lambda v_in, v_out:
                np.linalg.norm(v_in) > 1e-9 and np.linalg.norm(v_out) > 1e-9
                and float(np.dot(v_in, v_out) /
                          (np.linalg.norm(v_in) * np.linalg.norm(v_out)))
                < float(np.cos(np.radians(args.corner_angle)))
            )(pts_f64[i] - pts_f64[(i - 1) % len(pts_f64)],
              pts_f64[(i + 1) % len(pts_f64)] - pts_f64[i])
        )
    )
    print(f"Smooth Bezier segments: {len(bezier_segments)} "
          f"(tension={args.tension}, corners={n_corners})")
    ...
    # (b) Smooth-curve preview: sample the Bezier curves densely and draw
    ...
    for p1, cp1, cp2, p2 in bezier_segments:
    ...
    export_dxf_smooth(bezier_segments, p_dxf, image_height=H)
    export_svg_smooth(bezier_segments, (H, W), p_svg)
