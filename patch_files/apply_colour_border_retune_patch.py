...
def _sample_line_single(channel_2d: np.ndarray, p0: np.ndarray, p1: np.ndarray, half_width: int = 3, samples: int = 256) -> np.ndarray:
...
def _sample_line_mask(mask: np.ndarray, p0: np.ndarray, p1: np.ndarray, half_width: int = 2, samples: int = 256) -> np.ndarray:
...
def _side_evidence(gray: np.ndarray, p0: np.ndarray, p1: np.ndarray) -> tuple[float, float]:
    ...
    # mild gap tolerance: reward having at least one decent continuous run but don't
    # require perfection because creases/tape/glare can interrupt the border.
...
def _collect_candidate_quads(mask: np.ndarray, min_area_px: float, source: str) -> list[dict[str, Any]]:
...
def _broad_orange_mask(image_bgr: np.ndarray, orange_border_hex: str) -> np.ndarray:
    hsv = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2HSV)

    # Nominal digital orange window around the configured orange.
    h0, s0, v0 = _hex_to_hsv(orange_border_hex)

    # Printed/captured line is much duller and redder/browner than screen orange.
    # Build a tolerant union:
    masks = []

    # Broad terracotta / red-orange / brown-orange window from real samples.
    ...
    # Nominal configured orange, but with relaxed tolerance and low saturation floor.
    ...
    # Muted warm ink fallback, useful when white balance pushes the line toward pinkish brown.
    ...
    # Strengthen thin border lines while suppressing speckle.
...
def _build_candidates_from_masks(gray: np.ndarray, image_bgr: np.ndarray, border_px: int, min_area_px: float, border_colour_mode: str, orange_border_hex: str) -> tuple[list[dict[str, Any]], np.ndarray]:
    edges = _make_edge_mask(gray, border_px)
    dark_mask = _make_dark_mask(gray, border_px)

    orange_mask = _broad_orange_mask(image_bgr, orange_border_hex)
    orange_mask = _clear_image_border(orange_mask, border_px)

    combined = cv2.bitwise_or(edges, dark_mask)
    if border_colour_mode in {"orange", "auto"}:
    ...
    if border_colour_mode in {"orange", "auto"}:
        raw_candidates.extend(_collect_candidate_quads(orange_mask, min_area_px=min_area_px * 0.35, source="orange"))
...
def _mode_specific_colour_score(
    ...
    orange_score = float(np.mean(orange_side_scores))
    black_score = float(np.mean(black_side_scores))

    if requested_mode == "orange":
        return "orange", orange_score
    if requested_mode == "black":
        return "black", black_score

    # auto: whichever actually has stronger side evidence wins.
...
def detect_outer_border(
    ...
    *,
    ...
    candidates, orange_mask = _build_candidates_from_masks(
        gray=gray,
        image_bgr=image_bgr,
        border_px=border_px,
    ...
    for cand in candidates:
        ...
        detected_border_mode, colour_score = _mode_specific_colour_score(
            gray=gray,
            orange_mask=orange_mask,
            quad=quad,
            requested_mode=requested_colour_mode,
        )

        marker_support_score = _marker_support_score(
            image_bgr=image_bgr,
            quad=quad,
        ...
        if requested_colour_mode == "orange":
            plausible = bool(plausible and colour_score >= 0.10)
        ...
        # If orange is requested, punish oversized page-like candidates much harder.
        page_like_penalty = 0.0
        if requested_colour_mode == "orange":
            if area_frac > 0.66:
                page_like_penalty += (area_frac - 0.66) * 4.0
            if bbox_frac > 0.80:
                page_like_penalty += (bbox_frac - 0.80) * 3.5
            if colour_score < 0.18:
                page_like_penalty += (0.18 - colour_score) * 3.0
    ...
    if (
        best["score"] < 3.2
        or best["side_score"] < 0.22
        or best["ratio_score"] < 0.25
        or (requested_colour_mode == "orange" and best["colour_score"] < 0.18)
    ):
        confidence = "low"
    ...
    return BorderDetectionResult(
        ...
        diagnostics={
            ...
            "requested_colour_mode": requested_colour_mode,
