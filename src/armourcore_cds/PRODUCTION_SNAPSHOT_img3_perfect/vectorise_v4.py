...
ROUTE_PRESETS_V4: dict[str, dict] = {
    "pen": {
        ...
        "pre_close_px":           11,   # small kernel - keeps shapes distinct
        "pre_close_wide_px":      21,   # wide kernel - rescues big-gap shapes
        "skel_prune_spurs":       25,
        "bridge_max_gap_px":      55,
        "min_loop_perimeter_px":  150,
        "min_area_mm2":           80.0,
    ...
    **overrides,
    ...
    if hierarchy is not None and len(all_contours) > 0:
        ...
        for i, h in enumerate(hier):
            ...
            if kids:
                # Take the LARGEST hole - that's the shape interior.
                # Other holes are usually small inside-of-text artefacts.
                best = max(kids, key=lambda k: cv2.contourArea(all_contours[k]))
                chosen.append(all_contours[best])
            else:
                # No hole = solid blob = outer contour IS the shape
    ...
    for idx, pts in enumerate(loops):
        ...
        if len(simp) < 3:
            ...
            report.paths.append(diag)
            continue
        smoothed = chaikin_smooth(simp, iterations=cfg["smooth_iters"], closed=True)
        segs = _catmull_rom_to_bezier(smoothed, tension=1.0)

        diag.n_polyline_pts = len(smoothed)
        report.paths.append(diag)
